...
function FindOnPage(name, status) {
    ...
    {
        alert('Please, enter 3 more chars');
    ...
    if(input.length>=3)
    {
        function FindOnPageGo() {
            ...
            for(var i=0; i<result.length;i++) {
                result_arr[i] = result[i].replace(eval(search), '<span style="background-color:#29bff2; border-radius: 3px">'+input+'</span>'); //находим нужные элементы, задаем стиль и сохраняем в новый массив
            }
            for(var i=0; i<result.length;i++) {
